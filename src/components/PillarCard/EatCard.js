...
function EatCard() {
    ...
    const [eatScore, setEatScore] = useState(JSON.parse(localStorage.getItem("EatScore")) || []);
    ...
    const listItems = eatPillar.toDo.list.map((item, index) =>  (
        <div key = {index}>
            <div className="form-check">
                <input className="form-check-input" type="checkbox" onChange={handleCheckedBox} value={item} checked={eatScore.includes(item)}  />
    ...
    return (
        ...
        <div className="card animate__animated animate__fadeInUp h-100 pillarCard">
          <div className="card-body">
            <h4 className="card-title text-center pillarCard-title">{eatPillar.pillarName}</h4>
