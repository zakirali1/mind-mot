...
function SleepCard() {
    const [sleepScore, setSleepScore] = useState(JSON.parse(localStorage.getItem("SleepScore")) || []);
    ...
    const listItems = sleepPillar.toDo.list.map((item, index) =>  (
        <div key = {index}>
            <div className="form-check">
                <input className="form-check-input" type="checkbox" onChange={handleCheckedBox} value={item} checked={sleepScore.includes(item)}   />
    ...
    return (
        ...
        <div className="card animate__animated animate__fadeInUp h-100 pillarCard">
          <div className="card-body">
            <h4 className="card-title text-center pillarCard-title">{sleepPillar.pillarName}</h4>
